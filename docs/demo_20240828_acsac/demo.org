...
Clone the "Screaming Channels on Bluetooth Low Energy" repository (10 MB):
...
Leveraging the =Dockerfile=, we will setup a temporary Docker container for
reproducing the attacks. We will clean up the container at the end and no files
will be left on your system.
