...
- [[https://github.com/whad-team/whad-client][WHAD]] :: A framework for wireless hacking.
  It is registered as a submodule in =ext/whad= and can be initialized using =git submodule update --init --recursive ext/whad=.

Firmware:
- [[https://github.com/whad-team/butterfly][ButteRFly]] :: The attacker firmware, WHAD-enabled for low-level traffic injection. 
  It is registered as a submodule in =ext/butterfly= and can be initialized using =git submodule update --init --recursive ext/butterfly=.
- [[https://github.com/pierreay/bluescream-mynewt.git][MyNewt]] :: The victim firmware running the [[https://mynewt.apache.org/][NimBLE from Apache MyNewt]] Bluetooth Low Energy stack with a software AES (TinyAES) instead of using the hardware cryptoaccelerator.
  It is registered as a submodule in =ext/mynewt= and can be initialized using =git submodule update --init --recursive ext/mynewt=.
...
- [[https://github.com/pierreay/screaming_channels][screaming_channels]] :: Holds custom version of the Screaming Channels project, on which this one is based on.
  All credits for the initial project goes to [[https://github.com/eurecom-s3/screaming_channels][EURECOM]].
  It is registered as a submodule in =ext/screaming_channels= and can be initialized using =git submodule update --init --recursive ext/screaming_channels=.
- [[https://github.com/giocamurati/python_hel][Histogram-Enumeration Library (HEL)]] :: Key enumeration and estimation library.
  All credits goes to [[https://github.com/giocamurati][Giovanni Camurati]].
