...
# Google SCAAML code:
# https://raw.githubusercontent.com/google/scaaml/main/scaaml/capture/input_generators.py
...
def single_bunch(length: int,
    ...
    # Convert to np
    result: np.ndarray = np.array(transposed_result, dtype=np.int64)
...
def balanced_generator(length: int, bunches: int = 1, elements: int = 256):
...
def unrestricted_generator(length: int, bunches: int = 1, elements: int = 256):
...
# * Demonstration usage from Karel

if __name__ == "__main__":
    # Best case scenario 256*256 = 65000 examples
    for key in balanced_generator(length=16):  # length is the number of bytes (octets), so this is AES128
        for plaintext in balanced_generator(length=16):
            print(f"capture with {key = } and {plaintext = }")

    # Fewer examples, random key 100 * 256 examples
    for i, key in enumerate(unrestricted_generator(length=16)):
        if i > 100:
            break
        for plaintext in balanced_generator(length=16):
            # input of SBOX is still very nicely balanced.
            print(f"capture with {key = } and {plaintext = }")

    # Fixed key (least preferable for me)
    for plaintext in balanced_generator(length=16, bunches=256):
        print(f"capture with fixed key and {plaintext = }")
